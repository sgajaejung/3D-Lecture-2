...
struct sGlobal : public memmonitor::Monitor<sGlobal, TYPE_NAME(sGlobal)>
{
	...
	LPDIRECT3DVERTEXBUFFER9 vb2; // 버텍스 버퍼
	LPDIRECT3DINDEXBUFFER9 ib2; // 인덱스 버퍼
	int vtxSize2;
	int faceSize2;
	D3DMATERIAL9 mtrl2;
...
bool InitVertexBuffer()
{
	...
	ReadModelFile("../../media/teapot.dat",  global->vb2, global->vtxSize2, global->ib2, global->faceSize2);
	...
	ZeroMemory(&global->mtrl2, sizeof(global->mtrl2));
	global->mtrl2.Ambient = D3DXCOLOR(1,1,1,0.5f);
	global->mtrl2.Diffuse = D3DXCOLOR(1,1,1,0.5f);
	global->mtrl2.Specular = D3DXCOLOR(1,1,1,0.5f);
	global->mtrl2.Emissive = D3DXCOLOR(0,0,0,1);
	global->mtrl2.Power = 0.f;
...
void Render(int timeDelta)
{
		...
		)))
	{
		...
		// teapot 출력
		Matrix44 mt;
		mt.SetTranslate(Vector3(0, 0, 0));
		Matrix44 tm1 = mt;
		g_pDevice->SetTransform(D3DTS_WORLD, (D3DXMATRIX*)&tm1);

		g_pDevice->SetTexture(0, global->texture);
		g_pDevice->SetMaterial(&global->mtrl2);
		g_pDevice->SetStreamSource( 0, global->vb2, 0, sizeof(Vertex) );
		g_pDevice->SetIndices(global->ib2);
		g_pDevice->SetFVF( Vertex::FVF );
		g_pDevice->DrawIndexedPrimitive( D3DPT_TRIANGLELIST, 0, 0, global->vtxSize2, 0, global->faceSize2);
