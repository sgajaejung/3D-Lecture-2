...
void Matrix44::SetView( const Vector3& pos, const Vector3& dir0, const Vector3& up0 )
{
	...
	vDir = dir0.Normal(); // n (z̈)
	vCross = up0.CrossProduct( vDir ); //v (ẍ)
	vCross.Normalize();
	vUp = vDir.CrossProduct( vCross ); //u (ÿ)
...
{
	float fW = aspect * cosf( fov / 2.f ) / sinf( fov / 2.f );
	float fH = 1.0f    * cosf( fov / 2.f ) / sinf( fov / 2.f );
